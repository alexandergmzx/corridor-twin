...
5. **ADR 0024.2's pip-only claim is false today.** `scene.build` needs cv2
   ...
   and a genuinely pip-only CI job, or stop claiming CI coverage for the pip
   path. *Resolved on this branch: ADR 0025 decision 5 records the narrowed
   claim, commit 69f4b09 declares python3-opencv/python3-numpy in
   corridor_scene/package.xml (deliberately not pip — ABI risk), and both
   chains are pinned by transitive import-graph tests; the pip-only CI lane
   remains future work, unclaimed.*
...
18. Verify `omni.replicator` namespaces against the installed Isaac 5.1 before
    committing ADR 0024's dataset plan (CLAUDE.md installed-version
    rule).
...
| T0.4 | 0 | **[A]** Task-author ping (two lines, Aug 24) | Sent |
...
| 2 (~2 h) | Isaac publishing on a nonzero domain, live [F5b]; bridge throughput above 640×360 [F5c] | Publisher = the existing 0009 adapter with the v2 topic names (camera pose irrelevant to the crossing claim). 60 s capture in P's plane at 640×360/15 Hz, plus one 1280×720 trial for ADR 0024's resolution question: delivered-frame ratio, stamp monotonicity, /clock advancing in P's plane, stamp→receipt latency (same host), VRAM (nvidia-smi, house method), bridge CPU | Measured table for ADR 0026 |
...
re-enters at ADR 0024's resolution measurement, with the dual-context
alternative available as the recorded escape hatch.

## 6. Robot-A gate protocol (input to ADR 0022's run + ADR 0027)
...
3. **D3 — Speed policy: re-pin; neither ceiling moves.** The governor cap is a
   safety envelope with measured lineage [verification note: the governor
   defaults are tagged `[estimate]` in code — Yahboom-measured priors pending
   the fleet's P2 braking measurements on this chassis (governor.py:52-58);
   the conclusion stands and ADR 0023 records the accurate provenance] —
   untouchable; the Nav2 0.22 m/s cap
...
## 8. v2 requalification table (skeleton — no v1 number is quotable here)

Referenced by ADRs 0021/0022 as the table that replaces the v1 invariants
wholesale. Populated only by measured 0026/0027-and-later evidence; every cell
stays TBD until its named session runs.

| Metric | v2 value | Robot | Profile | Evidence |
|---|---|---|---|---|
| Isolation certificate | TBD (pass + mutation red) | — | all | ADR 0026 verification session |
| Nav2 goal result / map-frame error | TBD | per 0027 | nominal, wide | gate session bag |
| Matcher covariance at midpoint (robot2 runs) | TBD | robot2 | ×3 | degeneracy study |
| Max speed error — detector | TBD | per 0027 | nominal | Phase 3 report |
| Max speed error — ArUco baseline | TBD | per 0027 | nominal | Phase 3 report |
| Violations (count / station / exceedance) | TBD | per 0027 | nominal | Phase 3 report |
| P camera resolution / detection range per gate | TBD | — | nominal | ADR 0024 resolution measurement, bounded by the 0026 throughput ceiling |
| VRAM at chosen settings | TBD | — | nominal | session log |
| Route length / duration (emergent) | TBD per run | per 0027 | ×3 | run bags |
