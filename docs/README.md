...
| Phase 3 opener | **Closed — P watches from a 1.50 m mast on P's own footprint.** ADR 0019's screen blocks all five enforcement stations at P's standing height, so `corner_mast_over_the_screen` was ratified 2026-08-12 and implemented the same session: `police.camera_mast_height_m` authors the mast and `/World/Actors/PCameraMast/PCam` is the stage's single camera. [Decision memo](evidence/p_cam_candidates/NOTES.md) |
