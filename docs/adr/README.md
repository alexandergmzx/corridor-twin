...
| [0040](0040-corridor-sessions-are-udp-only.md) | Proposed (accepts on the T1 batch) | **Corridor sessions run DDS over UDP only** — the deafness 0039 could not explain is Fast DDS's documented SHM-after-churn failure (matched-but-silent; simctl's probe helpers `os._exit(0)` every ~10 s, and one teardown removed 85 stale segments). Fleet **OI-13** already decided UDP-only machine-wide at <1% measured rate cost; this bakes the export for corridor runs, both env names, before any participant exists. `CORRIDOR_DDS_PROFILE=` (empty) is the A/B control arm; `run.json` records `dds_profile`. The P-plane demo is NOT switched: ADR 0026's crossing ratios would need re-measuring first |
