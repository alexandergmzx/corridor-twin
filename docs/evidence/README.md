...
| Delivery day: the runs, and verifying the launch (2026-08-14) | Arena probe **PASS first attempt**; violation and compliant passes recorded from domain 43; certificate green with mutation red; the autonomous delivery reached **0.2251 m** inside the six-run band while FAILING two known gates. **The delivery run broke ADR 0037's correlation hours after it was accepted** — a lens created 71 s after `simctl start` went deaf anyway, which ADR 0039 answers. **Four verification runs then exercised the fixes live**: the restart-once rescued one deaf lens (**0.861**, the highest coverage measured), the second checkpoint REFUSED a run whose lens went deaf after passing its gate, and coverage was 0.818–0.886 on the three that ran. **Zero unwatchable successes** — but deafness still hit 2 of 4, and it remains unexplained | [Notes, per-run outcomes, the bring-up breakdown](ship-day/NOTES.md) |
| The deafness repro that did not reproduce (2026-08-14) | **Negative result, filed in bold**: three escalating churn arms on domain 69 — plain `os._exit` ×448, + victim kill-and-replace ×19 generations, + SIGKILL-mid-traffic (peak 68 `/dev/shm` segments) — and **zero deafness**. H1's synthetic form is bounded, not confirmed; the deafness criterion (count static ≥15 s while the publisher advanced ≥60) convicts nobody on silence alone. Cleanup swept only unmapped segments, the fleet's earned "stale" | [Notes, the three arms, the criterion](lens-deafness/NOTES.md) |
| Bring-up rework: T1 met — ten runs, zero deaf lenses (2026-08-14, ADRs 0040/0041) | **10/10 lens-healthy under UDP-only** (baseline 2 deaf of 4): frac 0.843–0.887, no restart-once, no refusal, `/dev/shm` census `pre` = `post` on every run (zero created; nine of the ten read 0/0/0 throughout, and run `151747` swept two pre-existing entries at teardown). **Rate cost none — odom_laser FASTER** (13.51–13.95 vs 10.57/10.61 Hz), EKF unchanged. Gate-green 4/10 vs 0/5 baseline; remaining reds are the pre-existing ADR-0029/0033 families, named so 10/10 is not read as a mission verdict. `simctl start` 63→53 s (`--no-rviz`); median command→motion ≈109 s pre-U8 vs ≈131 s baseline | [Notes, the ten-run table, the rate table](bringup-rework/NOTES.md) |
| The one-command entry point reproduces F3.1 (2026-08-17) | **`bash tools/demo.sh enforce` — the whole invocation.** Route **7.380 m, `reached_end=True`, 33.550 s** of sim time, identical to the recorded pass to three decimals; **371 `/p_cam/image_raw` delivered to P's plane** across the gateway (recorded: 376), `/clock` 2058 (2057), 3254 MiB, **one render product**, renderer mode read back rather than requested. The wrapper resolved robot1, the composed arena and the six-variable F3.1 environment itself, and took the Isaac lock `run_demo.sh` does not. **No speed or violation figure is quoted from it**: the live observer is v1's ArUco estimator and reports 0, because v2's verdicts come offline from this bag | [Notes, markers, bag counts](demo-entry-point/NOTES.md) |
