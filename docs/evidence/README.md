...
| Delivery day: the four runs (2026-08-14) | Arena probe **PASS first attempt**; violation and compliant passes recorded from domain 43; certificate green with mutation red; the autonomous delivery reached **0.2251 m** inside the six-run band while FAILING two known gates. **The delivery run broke ADR 0037's correlation hours after it was accepted** — a lens created 71 s after `simctl start` went deaf anyway, which ADR 0039 answers | [Notes, per-run outcomes](ship-day/NOTES.md) |
